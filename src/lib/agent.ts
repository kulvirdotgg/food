...
import type { Dish, ExploratoryLabel, HistorySummary } from "@/lib/types"
...
const recommendationPickSchema = z.object({
    ...
    reason: z
        ...
        .min(24)
        .max(200)
        .describe("One concise reason this dish fits its slot."),
})
...
const exploratoryPickSchema = recommendationPickSchema.extend({
    label: z
        ...
        .describe(
            "A practical UI card label that tells the user why they might want this dish.",
        ),
})

const recommendationOutputSchema = z.object({
    recommended: recommendationPickSchema.describe(
        "The single best low-friction recommendation.",
    ),
    alternatives: z
        .array(recommendationPickSchema)
        .length(3)
        .describe(
            "Three dependable backup recommendations with varied cuisines, formats, or flavor profiles.",
        ),
})
...
const explorationOutputSchema = z.object({
    dishes: z
        ...
        .length(3)
        .describe(
            "Three valid discovery picks that are meaningfully different from the normal recommendations.",
        ),
})

export type RecommendedSelectionResult = z.infer<
...
- Use the 30-day history as a soft preference to avoid stale patterns.
    `.trim(),
        ...
        output: Output.object({
            ...
            description:
                "A structured recommended dish and three alternatives selected from the eligible catalog.",
        }),
    })
}
...
- Across the three exploratory picks, prefer different label types when accurate.
    `.trim(),
        ...
        output: Output.object({
            ...
            description:
                "Three discovery picks selected from the eligible catalog while avoiding the normal set.",
        }),
    })
}

export type RecommendationDishSelector = DishSelector<
    RecommendedDishSelectorInput,
    RecommendedSelectionResult
>

export class recommendationDishSelector implements RecommendationDishSelector {
    constructor(
        private readonly agent: ReturnType<
            typeof recommendationAgent
        > = recommendationAgent(),
    ) {}

    async select(
        input: RecommendedDishSelectorInput,
    ): Promise<RecommendedSelectionResult> {
...
export type ExploratoryDishSelector = DishSelector<
    ExplorationDishSelectorInput,
...
export class exploratoryDishSelector implements ExploratoryDishSelector {
    constructor(
        private readonly agent: ReturnType<
            typeof explorationRecommendationAgent
        > = explorationRecommendationAgent(),
    ) {}

    async select(
        input: ExplorationDishSelectorInput,
    ): Promise<ExplorationSelectionResult> {
        const result = await this.agent.generate({
            prompt: [
